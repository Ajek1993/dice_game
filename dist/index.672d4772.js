...
const findFigure = (nums)=>{
    ...
    for(let i = 0; i < countNumbers.length; i++){
        if (countNumbers.indexOf(2) && countNumbers.lastIndexOf(2) && countNumbers.indexOf(2) !== countNumbers.lastIndexOf(2)) {
            pointsPerRound += (countNumbers.indexOf(2) + 1 + (countNumbers.lastIndexOf(2) + 1)) * 3;
            return `Dwie pary z ${countNumbers.indexOf(2) + 1} oraz ${countNumbers.lastIndexOf(2) + 1}`;
        }
        if (countNumbers.includes(2) && countNumbers.includes(3)) {
            pointsPerRound += (countNumbers.indexOf(2) + 1 + (countNumbers.indexOf(3) + 1)) * 3;
            return `Full z ${countNumbers.indexOf(2) + 1} oraz ${countNumbers.indexOf(3) + 1}`;
        }
        ...
        if (countNumbers.includes(3)) {
            ...
            return "Tr\xf3jka z " + (countNumbers.indexOf(3) + 1);
        }
...
const fillTable = (scoreNumbers)=>{
    ...
    newTr.lastElementChild.previousElementSibling.innerText = findFigure(scoreNumbers);
